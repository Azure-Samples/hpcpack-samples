...
namespace SampleBroker
{
    ...
    internal sealed class ServiceClientManager : ISchedulerNotify
    {
        ...
        /// <summary>
        /// Redirect request to a dispatcher
        /// </summary>
            ...
            while (true)
            {
                if (this.dispatcherList.Count > 0)
                {
                    lock (this.dispatcherList)
                    {
                        if (this.dispatcherList.Count > 0)
                        {
                            Dispatcher dispatcher = this.dispatcherList.Values[this.dispatcherList.Count - 1];
                ...
                foreach (TaskInfo info in taskInfoList)
                {
                    switch (info.State)
                    {
                        ...
                        case TaskState.Running:
                            ...
                            {
                                this.dispatcherList[info.Id] = new Dispatcher(this.sessionId, info.Id, info.Capacity, info.MachineName);
                            }
